...
export default function TeamLeaderDashboard() {
  ...
  const [teamAgents, setTeamAgents] = useState<AppUser[]>([]);
  ...
  useEffect(() => {
    ...
    // Subscribe to teams
    const teamsUnsub = onSnapshot(query(collection(db, 'teams'), orderBy('name')), (snap) => {
      const teams = snap.docs.map(d => ({ id: d.id, ...d.data() } as Team));
      // Find team where user is the leader
      const userTeam = teams.find(t => t.leaderId === currentUser?.id);
  ...
  // Subscribe to team agents when team is found
  useEffect(() => {
    if (!myTeam) return;
    ...
    const agentsUnsub = onSnapshot(
      ...
      (snap) => {
        const allAgents = snap.docs.map(d => ({ id: d.id, ...d.data() } as AppUser));
        // Filter to only agents in the user's pod
        const podAgents = allAgents.filter(a => a.podId === myTeam.podId);
        setTeamAgents(podAgents);
      }
    );
  ...
  const teamAchievements = useMemo(() => {
    return achievements.filter(a => teamAgentIds.includes(a.agentId));
  }, [achievements, teamAgentIds]);
  ...
  const activeCompetitions = useMemo(() => {
    ...
    return competitions.filter(c => {
      ...
      const podInCompetition = c.podIds?.includes(myTeam?.podId || '');
      return start && end && now >= start && now <= end && podInCompetition;
    });
  }, [competitions, myTeam?.podId]);
  ...
  }, [teamAgents, teamAchievements]);

  const teamRank = useMemo(() => {
    const totalAchievements = teamAchievements.length;
    // This would need comparison with other teams - simplified for now
    ...
    <div className="pt-6 space-y-8">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <Card variant="glass">
          ...
          <CardContent>
            <div className="text-2xl font-bold">{myTeam?.name || 'No Team'}</div>
      ...
      <Card variant="glass">
        ...
        <CardContent>
          {!myTeam ? (
            <p className="text-muted-foreground text-center py-8">No team assigned</p>
