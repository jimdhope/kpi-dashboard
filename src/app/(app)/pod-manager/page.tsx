...
export default function PodManagerDashboard() {
  ...
  useEffect(() => {
    ...
    const podsUnsub = onSnapshot(query(collection(db, 'pods'), orderBy('name')), (snap) => {
      const allPods = snap.docs.map(d => ({ id: d.id, ...d.data() } as Pod));
      // Filter to only pods managed by this user
      const userPodIds = currentUser?.podId ? [currentUser.podId] : [];
      const managedPods = allPods.filter(p => p.managerId === currentUser?.id || userPodIds.includes(p.id));
      setMyPods(managedPods);
    });
    ...
    // Subscribe to agents
    const agentsUnsub = onSnapshot(query(collection(db, 'users'), where('roles', 'array-contains', 'agent'), orderBy('name')), (snap) => {
      setAgents(snap.docs.map(d => ({ id: d.id, ...d.data() } as AppUser)));
    });
    unsubscribes.push(agentsUnsub);
  ...
  }, [currentUser?.id, currentUser?.podId]);
  ...
  const myTeams = useMemo(() => {
    return teams.filter(t => myPodIds.includes(t.podId));
  }, [teams, myPodIds]);
  ...
  const activeCompetitions = useMemo(() => {
    ...
    return competitions.filter(c => {
      ...
      const podInCompetition = c.podIds?.some(id => myPodIds.includes(id));
      return start && end && now >= start && now <= end && podInCompetition;
    });
  }, [competitions, myPodIds]);
          ...
          {myPods.length === 0 ? (
            ...
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
              {myPods.map((pod) => {
                ...
                const podTeams = myTeams.filter(t => t.podId === pod.id);
                return (
