import { z } from "zod";
import { TEAMS_AUTOMATION_DELIVERY_FORMATS, TEAMS_CHANNEL_CATEGORIES } from "@/lib/contracts";
...
export async function GET() {
  try {
    return ok(await teamsMessageTemplateService.listTemplates());
  } catch (error) {
...
export async function POST(request: Request) {
  try {
    ...
    return ok(await teamsMessageTemplateService.createTemplate(payload), { status: 201 });
  } catch (error) {
    ...
    return errorResponse(500, "Failed to create Teams message template.");
  }
}
