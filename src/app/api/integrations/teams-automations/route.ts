...
export async function GET() {
  try {
    return ok(await teamsAutomationService.listAutomations());
  } catch (error) {
...
export async function POST(request: Request) {
  try {
    ...
    return ok(
      await teamsAutomationService.createAutomation({
        ...payload,
    ...
    return errorResponse(500, "Failed to create Teams automation.");
  }
}
