...
export async function GET(request: Request, context: { params: Promise<{ code: string }> }) {
  try {
    ...
    const token = await quizShowService.getDisplayLinkToken(user.id, normalizedCode);
    const baseUrl = process.env.PUBLIC_URL || process.env.APP_URL || new URL(request.url).origin;
    return ok({ url: `${baseUrl}/mini-games/quiz-show/display?code=${encodeURIComponent(normalizedCode)}&token=${encodeURIComponent(token)}` });
  } catch (error) {
