...
export async function GET() { try { await requireManage(); return ok({ quizzes: await quizShowService.listQuizzes() }); } catch (error) { return failure(error, "Unable to load quizzes."); } }
export async function POST(request: Request) { try { const user = await requireManage(); return ok({ quiz: await quizShowService.createQuiz(user.id, quizSchema.parse(await request.json())) }, { status: 201 }); } catch (error) { return failure(error, "Unable to create quiz."); } }
