...
type Feedback = { id: string; subject: string; message: string; status: string; createdAt: string };
...
export default function FeedbackPage() {
  ...
  return <main className="mx-auto w-full max-w-2xl space-y-6 p-4 sm:p-6">
    ...
    {feedback.length > 0 && <Card><CardHeader><CardTitle className="text-base">Your submissions</CardTitle></CardHeader><CardContent className="space-y-3">{feedback.map((item) => <div key={item.id} className="rounded-lg border p-3"><div className="flex items-center justify-between gap-2"><p className="font-medium">{item.subject}</p><span className="text-xs text-muted-foreground">{item.status.replace("_", " ")}</span></div><p className="mt-1 whitespace-pre-wrap text-sm text-muted-foreground">{item.message}</p><p className="mt-2 text-xs text-muted-foreground">{new Date(item.createdAt).toLocaleString()}</p></div>)}</CardContent></Card>}
  </main>;
}
