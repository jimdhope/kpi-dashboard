...
import { Trophy, Target, BarChart3, Gamepad2, User, ChevronDown, Shield } from 'lucide-react';
...
const roleViews: RoleView[] = [
  ...
  { id: 'podSupport', label: 'Pod Support', href: '/pod-support', icon: Trophy, description: 'Support pod members' },
  { id: 'podManager', label: 'Pod Manager', href: '/pod-manager', icon: Trophy, description: 'Manage pods and teams' },
  { id: 'teamLeader', label: 'Team Leader', href: '/team-leader', icon: Trophy, description: 'Lead your team' },
...
const navItems: { label: string; href: string; icon: React.ElementType }[] = [
  { label: 'Competitions', href: '/agent/competitions', icon: Trophy },
  { label: 'Trackers', href: '/agent/trackers', icon: Target },
  { label: 'Performance', href: '/agent/performance', icon: BarChart3 },
  { label: 'Mini Games', href: '/agent/mini-games', icon: Gamepad2 },
];

export function AgentNavBar({ className }: { className?: string }) {
  const pathname = usePathname();
  const router = useRouter();
  const [currentUser, setCurrentUser] = React.useState<AppUser | null>(null);
  const [currentView, setCurrentView] = React.useState<string>('agent');
  ...
  const availableViews = React.useMemo(() => {
    if (!currentUser?.roles) return [roleViews[1]];
    
    const userRoles = currentUser.roles;
    return roleViews.filter(view => userRoles.includes(view.id));
  }, [currentUser?.roles]);

  const currentViewInfo = roleViews.find(v => v.id === currentView) || roleViews[1];
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-48">
              ...
              <DropdownMenuItem 
                ...
                className="cursor-pointer text-white hover:bg-red-600 focus:bg-red-600 focus:text-white"
              >
                Logout
              </DropdownMenuItem>
            </DropdownMenuContent>
