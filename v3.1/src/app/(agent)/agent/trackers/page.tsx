...
import type { AppUser, TrackerKpiRecord, PerformanceLogRecord } from '@/lib/contracts';
...
export default function AgentTrackersPage() {
  ...
  const [trackerKpis, setTrackerKpis] = useState<TrackerKpiRecord[]>([]);
  ...
  useEffect(() => {
    async function fetchTrackerLogs() {
      ...
      try {
        ...
        const res = await fetch(`/api/performance/logs?date=${dateStr}`);
        if (res.ok) {
          const data = await res.json();
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Popover>
          ...
          <PopoverContent className="w-auto min-w-[300px] p-0" align="end">
            <Calendar
              mode="single"
            ...
            {isLoadingData ? (
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                {trackerKpis
                  .map(kpi => ({
                    ...kpi,
                    initials: kpi.name.substring(0, 2).toUpperCase(),
                    ...
                    <tr className="border-b border-glass-border/30">
                      ...
                      {trackerKpis.slice(0, 3).map(kpi => (
                        <th key={kpi.id} className="text-center py-2 px-1 font-medium" title={kpi.name}>
                          {kpi.name.substring(0, 2).toUpperCase()}
                        </th>
                      ))}
