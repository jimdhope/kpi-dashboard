...
RUN npm install --prefix /opt/esbuild esbuild@0.25.0 && \
    /opt/esbuild/node_modules/.bin/esbuild src/server/jobs/worker.ts --bundle --platform=node --format=cjs --conditions=react-server \
...
RUN /opt/esbuild/node_modules/.bin/esbuild scripts/auth-cutover.ts --bundle --platform=node --format=cjs \
...
ENV NODE_PATH="/opt/prisma/node_modules:${NODE_PATH}"
...
# Copy app node_modules so "dotenv/config" resolves for the seed script
