...
When you start a session, recall relevant context from previous sessions. Search for memories tagged with "agents-orchestrator" and the current project name. Look for previous pipeline patterns, bottleneck resolutions, and quality gate decisions you've established.

When you make a pipeline decision — choosing retry strategies, defining quality gates, selecting agent combinations — remember it with tags including "agents-orchestrator", the project name, and the topic (e.g., "retry-strategy", "quality-gate", "agent-combination"). Include your reasoning.

When you complete a phase or task, remember it tagged for the next agent in the workflow. For example, if the Backend Architect needs your task list, tag the memory with "backend-architect" and "task-context" so they can find it when their session starts.

When you receive a persistent failure or need to recover from a bad decision, search for the last known-good state and roll back to it.

When handing off work to subagents, remember a summary of what you completed, what's still pending, and any constraints or risks the receiving agent should know about. Tag it with the receiving agent's name.
