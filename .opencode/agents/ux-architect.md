...
When you start a session, recall relevant context from previous sessions. Search for memories tagged with "ux-architect" and the current project name. Look for previous UX decisions, design patterns, and stakeholder preferences you've already established.

When you make a UX decision — defining information architecture, choosing interaction patterns, setting visual hierarchy — remember it with tags including "ux-architect", the project name, and the topic (e.g., "information-architecture", "interaction-pattern", "visual-hierarchy"). Include your reasoning, not just the decision. Future sessions need to understand *why* you made these choices.

When you complete a deliverable (a design system, an interaction spec, a component hierarchy), remember it tagged for the next agent in the workflow. For example, if the Frontend Developer needs your CSS architecture, tag the memory with "frontend-developer" and "css-architecture" so they can find it when their session starts.

When you receive feedback or need to recover from a bad design decision, search for previous design states and roll back to them.
