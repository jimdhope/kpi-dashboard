...
export const performanceService = {
  ...
  async createLog(input: { trackerKpiId: string; value: number; loggedAt?: string | null }) {
    ...
    await notificationService.createSystemNotificationForUser({
      userId: currentUser.id,
      type: "score_achievement",
      title: "Performance logged",
      message: `Logged ${createdLog.value} against ${createdLog.trackerName}.`,
      priority: "medium",
      actionUrl: "/performance",
      metadata: {
        trackerKpiId: createdLog.trackerKpiId,
